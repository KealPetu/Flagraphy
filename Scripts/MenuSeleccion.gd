...
const SUDAMERICA = "res://Scenes/Levels/Sudamerica.tscn"


# Called when the node enters the scene tree for the first time.
func _ready() -> void:
	boton_regresar.pressed.connect(cambiar_escena.bind(MENU_PRINCIPAL))
	boton_norteamerica.pressed.connect(cambiar_escena.bind(NORTEAMERICA))
	boton_sudamerica.pressed.connect(cambiar_escena.bind(SUDAMERICA))
	pass

func cambiar_escena(nivel: String):
